...
Ask naturally: `play Kind of Blue`, `pause the music`, `resume the music`, `next song`, or `previous song`. When Bluetooth headphones are connected, every new play request asks whether to use **Headphones** or **Living Room Apple TV**. Choose in the Music widget or answer with `headphones`, `my PC`, `TV`, or `Living Room Apple TV`. Headphones use normal playback from the Windows Apple Music app. If Bluetooth headphones are not connected, Osun selects Living Room Apple TV automatically. You can also name the destination in the original command: `play Discovery on headphones` or `play Discovery on the TV`.

Short commands are supported: `play` means play music, while a Music-agent phrase such as `a Cardi B song` or `anything` fills the song request conversationally. A newer music command supersedes an older unanswered device question, preventing stale playback requests from returning later.

Ask `what devices are available to play on?`, `where can I play Apple Music?`, or `list my music devices` to open the Music widget's read-only device inventory. Osun lists the currently usable destinations: Headphones appear only while Windows exposes a connected Bluetooth headphone output, and Living Room Apple TV appears while the protected Home Assistant connection is configured. Looking at the inventory never starts playback and does not discard a song waiting for a destination.

For real headphone playback, open the full Windows **Apple Music** app, sign in, and play one song normally. Then open **Settings -> Music agent**, choose **Windows app**, save, and select **Test Apple Music app**. The test reports the Apple Music control state, connected Bluetooth headphones, and Apple TV availability. No Apple Developer Program membership or developer token is required. Keep Osun and Apple Music under the same Windows user and privilege level; do not run either as Administrator.

Living Room Apple TV playback uses the existing DPAPI-protected Home Assistant credential. In **Settings -> Music agent**, select **Discover media centers**, choose the Home Assistant `media_player` entity for your Apple TV, and save. Osun then allowlists only that persisted entity, sends the validated Apple Music catalog link through `media_player.play_media`, and reads its state back. This launches native Apple Music content on the Apple TV instead of mirroring encrypted PC audio through an unsupported Windows AirPlay control. Make sure the Home Assistant Apple TV integration is paired before discovery. Osun still cannot take over an arbitrary iPhone or HomePod. MusicKit remains optional; never paste an Apple private `.p8` signing key into Osun. See the [P2 Music contract](P2_APPLE_MUSIC_AGENT.md) for the full boundary and canary steps.
...
- Remove local Home Assistant authority: Settings → **Delete light token**. This disables live lighting and Living Room Apple TV playback.
