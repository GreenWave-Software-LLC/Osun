...
Ask naturally: `play Kind of Blue`, `pause the music`, `resume the music`, `next song`, or `previous song`. If no registered device has successfully played music in the last five minutes, Osun asks where to play and the compact Music widget offers **This PC**. You can answer in chat with `my PC`, `on my PC`, `this computer`, or `agent box`; Osun keeps the pending song request, selects This PC, and plays it. After successful playback, another request within 300 seconds routes to that device automatically. At 301 seconds, Osun asks again. You can also name the device in the original command: `play Discovery on my PC`.
...
Ask `what devices are available to play on?`, `where can I play Apple Music?`, or `list my music devices` to open the Music widget's read-only device inventory. Osun lists only enabled, registered playback devices and shows whether each was recently active. Looking at the inventory never starts playback and does not discard a song that is waiting for a device choice.
