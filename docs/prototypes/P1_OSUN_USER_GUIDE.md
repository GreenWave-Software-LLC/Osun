...
Living Room Apple TV playback uses the existing DPAPI-protected Home Assistant credential and only the exact `media_player.living_room_apple_tv` entity, or one entity whose friendly name exactly matches **Living Room Apple TV**. Osun sends the validated Apple Music catalog link through `media_player.play_media` and reads the Apple TV media-player state back. This launches native Apple Music content on the Apple TV instead of mirroring encrypted PC audio through an unsupported Windows AirPlay control. Make sure the Home Assistant Apple TV integration is paired before testing. Osun still cannot take over an arbitrary iPhone or HomePod. MusicKit remains optional; never paste an Apple private `.p8` signing key into Osun. See the [P2 Music contract](P2_APPLE_MUSIC_AGENT.md) for the full boundary and canary steps.
